...
class UserService(
	...
	fun update(oldUser: UserModel): UserModel {
		...
		if (userModel != null) {
			...
			userModel.driver?.prices?.pricePerKm =
				if (oldUser.driver?.prices?.pricePerKm != null) oldUser.driver?.prices?.pricePerKm!! else 10f
			...
			userModel.driver!!.isWorking =
				if (userModel.type == Static.CLIENT_TYPE) false else oldUser.driver!!.isWorking
	...
	private fun updateLanguages(
		oldLanguagesList: List<SpeakingLanguagesModel>,
		newLanguagesList: List<SpeakingLanguagesModel>,
		user: UserModel
	): List<SpeakingLanguagesModel> {
	...
	private fun saveLanguage(language: String = "sr", userModel: UserModel): SpeakingLanguagesModel {
		val savedLang =
			languageService.save(SpeakingLanguagesModel(language = language, userId = userModel.id, user = userModel))
